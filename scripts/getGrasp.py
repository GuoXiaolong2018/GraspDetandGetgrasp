...
import os
os.environ["LD_LIBRARY_PATH"] += os.pathsep + "/home/gxl/Downloads/Dobot-Magician-ROS-Demo/dobot_ws/src/dobot/src/DobotDll_x64"
import rospy
...
if __name__ == "__main__":
    ...
    with open('/home/gxl/catkin_ws/src/grasp_detection/scripts/calibration_results.json','r') as f:
        p = json.load(f)
        #print(p)
    ...
    if (state == dType.DobotConnect.DobotConnect_NoError):
        ...
        homex = 178;homey = -204;homez = 40;homer = 0;
        #xyzvel = 200;xyzacc = 200;rvel = 200;racc = 200;


        x1=TARGET[0,0];y1=TARGET[1,0];z1=homez;r1=check_angle(a.angle+90);#目标上方#待定 #a.angle+90 #-237~63
        ...
        #从家出发#移动到home处#
        #dType.SetPTPCmd(api, 2, homex, homey, homez, homer, isQueued=1)
        #dType.dSleep(ms=2000) 

        #移动到目标上方x1
        dType.SetPTPCmd(api, 1, x1, y1, z1, r1, isQueued=1)
        dType.dSleep(ms=2000) 
        ...
        #移动到目标处
        #移动到x2处#
        dType.SetPTPCmd(api, 2, x2, y2, z2, r2, isQueued=1)
        dType.dSleep(ms=2000) 
        ...
        #移动到目标上方x1
        #移动到x1处#
        dType.SetPTPCmd(api, 2, x1, y1, z1, r1, isQueued=1)
        dType.dSleep(ms=2000)

        #移动到箱子上方
        #移动到x3处#
        dType.SetPTPCmd(api, 2, x3, y3, z3, r3, isQueued=1)
        dType.dSleep(ms=2000)

        #移动到箱子里x4处#
        dType.SetPTPCmd(api, 2, x4, y4, z4, r4, isQueued=1)
        dType.dSleep(ms=2000)
        dType.SetEndEffectorGripper(api, enableCtrl=1,  on=0, isQueued=1)#爪子张开#
        dType.dSleep(ms=2000)
